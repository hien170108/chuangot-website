    ...
    add: async (productData) => {
      try {
        ...
        const newProduct = {
          ...
          category: productData.category, // fixed: use .category not .category_id
          description: productData.description,
          longDescription: productData.longDescription, // fixed: use .longDescription not .long_description
          nutrition: {
            calories: productData.nutritionCalories,
            fat: productData.nutritionFat,
            saturatedFat: productData.nutritionSaturatedFat,
            cholesterol: productData.nutritionCholesterol,
            sodium: productData.nutritionSodium,
            carbs: productData.nutritionCarbs,
            sugar: productData.nutritionSugar,
            protein: productData.nutritionProtein,
            calcium: productData.nutritionCalcium,
            vitaminD: productData.nutritionVitaminD,
          },
        }
    ...
    getAll: async () => {
      try {
        ...
        const safeUsers = users.map((user) => ({
          id: user.id,
          username: user.username,
          fullName: user.fullName, // fixed: use fullName for consistency
          role: user.role,
          email: user.email,
          avatar: user.avatar,
        }))
    ...
    add: async (userData) => {
      try {
        ...
        const safeUser = {
          ...
          fullName: newUser.fullName, // fixed
          role: newUser.role,
          email: newUser.email,
      ...
      try {
        ...
        if (index !== -1) {
          ...
          const safeUser = {
            ...
            fullName: users[index].fullName, // fixed
